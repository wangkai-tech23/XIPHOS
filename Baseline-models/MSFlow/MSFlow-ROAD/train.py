...
def model_forward(c, extractor, parallel_flows, fusion_flow, image):
    h_list = extractor(image) # h_list = [ (512,256,5,5), (512,512, 3,3), (512,1024,2,2) ] # 现在是  # h_list = [ (8,256,128,128), (8,512, 64,64), (8,1024,32,32) ] # 现在是
    if c.pool_type == 'avg':
        pool_layer = nn.AvgPool2d(3, 2, 1)
        # pool_layer = nn.AvgPool2d(5, 3, 1)
    elif c.pool_type == 'max':
        pool_layer = nn.MaxPool2d(3, 2, 1)
        # pool_layer = nn.MaxPool2d(5, 3, 1)
    else:
        pool_layer = nn.Identity()

    z_list = []
    parallel_jac_list = []  #h:(512,256,5,5) #y:(512,256,3,3) #cond:(512,64,3,3)
    for idx, (h, parallel_flow, c_cond) in enumerate(zip(h_list, parallel_flows, c.c_conds)):
        ...
        # cond = positionalencoding2d(c_cond, 1, 1).to(c.device).unsqueeze(0).repeat(B, 1, 1, 1) # (8,64,64,64)
...
def train_meta_epoch(c, epoch, loader, extractor, parallel_flows, fusion_flow, params, optimizer, warmup_scheduler, decay_scheduler, scaler=None):
    ...
    for sub_epoch in range(c.sub_epochs): # range(0,25)
        ...
        # for idx, (image, _, _) in enumerate(loader):
            ...
            optimizer.zero_grad()
            image = image.to(c.device) #（8,3,512,512） bath=8,so, our (8,3,9,9)   #now :(512,3,9,9)
            if scaler: # None
                with autocast():
...
def train(c):
    ...
    image_path = os.path.join(data_root,"StatGraph/BaselineModels/ROAD9_9_3-each27") #ROAD_small  # data9_9_3-each27-alltest
    print(image_path)
    ...
    if c.mode == 'test':
        ...
        roc_test = roc_auc_score(np.asarray(gt_label_list, dtype=int), preds)
        ...
        roc_test = roc_auc_score(np.asarray(gt_label_list, dtype=int), preds)
        # roc_test = roc_auc_score(np.asarray(gt_label_list, dtype=int), anomaly_score)
        print("-----------\n The ROC score on CHD dataset is {}".format(
            roc_test))  # The ROC score on SWaT dataset is 0.639797355039688
