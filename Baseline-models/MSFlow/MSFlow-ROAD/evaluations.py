...
def eval_det_loc(det_auroc_obs, loc_auroc_obs, loc_pro_obs, epoch, gt_label_list, anomaly_score, gt_mask_list, anomaly_score_map_add, anomaly_score_map_mul, pro_eval):
    gt_label = np.asarray(gt_label_list, dtype=np.bool) # 83个true or false
    gt_mask = np.squeeze(np.asarray(gt_mask_list, dtype=np.bool), axis=1) #(83,9,9)  # gt_mask_list：83个 (8,1,9,9)
    det_auroc, best_det_auroc = eval_det_auroc(det_auroc_obs, epoch, gt_label, anomaly_score)
    loc_auroc =0;  best_loc_auroc = False # eval_loc_auroc(loc_auroc_obs, epoch, gt_mask, anomaly_score_map_add)
    if pro_eval:
