...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.conv1(x)  #8,64,5,5
        x = self.bn1(x)
        x = self.relu(x)
        # x = self.maxpool(x)  # 8,64,3,3

        x1 = self.layer1(x) #8,256,5,5
        x2 = self.layer2(x1) #8,512,3,3
        x3 = self.layer3(x2)  #8,1024,2,2
        # x = self.layer4(x)
        # remove extra layers
        #x = self.avgpool(x)
        #x = torch.flatten(x, 1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        return self._forward_impl(x)  # 8,3,9,9
