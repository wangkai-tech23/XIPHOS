...
class ScaleDotProductAttention(nn.Module):
    ...
    def forward(self, x,mask=None, e=1e-12):
        ...
        shape = x.shape
        x_shape = x.reshape((shape[0],shape[1], -1)) #(512,9,27)
...
class MTGFLOW(nn.Module):
    ...
    def test(self, x, ):
        # x: N * K * L * D
        full_shape = x.shape #(512,9,27,1）
        graph,_ = self.attention(x) #(512,9,9）,_ = k = (512,9,9)
        self.graph = graph
        # reshape: N*K, L, D
        x = x.reshape((x.shape[0]*x.shape[1], x.shape[2], x.shape[3])) # (4608,27,1)
        h,_ = self.rnn(x) # (4608,9,32)

        # resahpe: N, K, L, H
        h = h.reshape((full_shape[0], full_shape[1], h.shape[1], h.shape[2]))  #(512,9,27,32)
        h = self.gcn(h, graph)  #(512,9,27,32)

        # reshappe N*K*L,H
        h = h.reshape((-1,h.shape[3])) #(124416,32)
        x = x.reshape((-1,full_shape[3]))  # (4608,27,1)
        log_prob = self.nf.log_prob(x, full_shape[1], full_shape[2], h).reshape([full_shape[0],-1]) #(512,243)
        log_prob = log_prob.mean(dim=1) #(512,)
