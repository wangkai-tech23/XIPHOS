...
class MAF(nn.Module):
    ...
    def base_dist(self, z, k, window_size):
        # C = z.shape[-1]  #C = 51
        # z.shape = 124416=512*9*27
        N =  z.shape[0]//k//window_size
        # bb = self.base_dist_mean.repeat_interleave(1, 0) #(6561,1)   #27-65651, 9-2187
        # cc = bb.repeat(N,1)
        # aa = z-cc
        logp = - 0.5*(z-self.base_dist_mean.repeat_interleave(1, 0).repeat(N,1))**2
