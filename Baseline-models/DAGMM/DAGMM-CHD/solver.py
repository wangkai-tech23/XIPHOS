...
class Solver(object):
    ...
    def train(self):
        ...
        for e in range(start, self.num_epochs):
            for i, (input_data, labels) in enumerate(tqdm(self.data_loader)):
                ...
                input_data = self.to_var(input_data)  #(input_data, labels) : (1024,118),(1024,)
                input_data = input_data.reshape(input_data.shape[0], -1)

                total_loss,sample_energy, recon_error, cov_diag = self.dagmm_step(input_data) #真正的训练
                # Logging
                loss = {}
    ...
    def test(self):
        ...
        non_zero_indices = np.nonzero(test_energy)
        test_energy[non_zero_indices] = 1


        test_z = np.concatenate(test_z,axis=0)
        test_labels = np.concatenate(test_labels,axis=0)
        non_zero_indices = np.nonzero(test_labels)
        test_labels[non_zero_indices] = 1
        ...
        roc_test = roc_auc_score(np.asarray(gt, dtype=int), pred)

        print("Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f}, ROC-AUC : {:0.4f}".format(accuracy, precision, recall, f_score, roc_test))
