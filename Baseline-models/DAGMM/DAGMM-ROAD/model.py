...
class Cholesky(torch.autograd.Function):
    def forward(ctx, a):
        l = torch.cholesky(a, False) #torch.potrf(a, False) #
...
class DaGMM(nn.Module):
    ...
    def forward(self, x): #x 1024,243

        enc = self.encoder(x)  # 1024,1

        dec = self.decoder(enc) # 1024,243

        rec_cosine = F.cosine_similarity(x, dec, dim=1) # 1024
        rec_euclidean = self.relative_euclidean_distance(x, dec) # 1024

        z = torch.cat([enc, rec_euclidean.unsqueeze(-1), rec_cosine.unsqueeze(-1)], dim=1)  # 1024,3

        gamma = self.estimation(z) #1024,4
    ...
    def compute_energy(self, z, phi=None, mu=None, cov=None, size_average=True):
        ...
        # K
        det_cov = torch.cat(det_cov).to(device)#.cuda()
