...
from dataset1 import load,load_data,load_normal_data,load_extra_data
...
def local_graph_loss_(l_enc1, l_enc2, batch, measure, mask):
    ...
    num_graphs = l_enc2.shape[0]  # 32
    num_nodes = l_enc1.shape[1]  # 50
    num_feat = l_enc1.shape[2] # 64       # max_nodes = num_nodes // num_graphs
    ...
    neg_mask = torch.ones((num_graphs*num_nodes, num_graphs*num_nodes))-pos_mask
...
def execute_test(dataset, gpu, num_layer=4, epoch=40, batch=64):
    ...
    model.load_state_dict(torch.load(f'{hid_units}_{batch_size}_IVN_{max_nodes}_ROAD_upgrade_last2_edge.pkl'))
    model.eval()
    ...
    write_path = 'upgrade/last/ROAD_' + str(hid_units) + '_embeds_' + str(max_nodes) + 'test.csv'
...
def execute_test0(dataset, gpu, num_layer=4, epoch=40, batch=64):
    ...
    adj, diff, feat, labels, num_nodes = load_extra_data(dataset, 'test')
    ...
    write_path = 'upgrade/last/ROAD_' + str(hid_units) + '_embeds_' + str(max_nodes) + 'test0.csv'
    np.savetxt(write_path, embeds.cpu().detach().numpy(), fmt='%.2f', delimiter=',')
    time_end = time.time()
    print('- ' * 5, 'Over embend', '- ' * 5, 'with time:', time_end - time_start)
