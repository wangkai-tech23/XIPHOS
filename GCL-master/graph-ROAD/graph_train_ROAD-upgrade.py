...
from dataset1 import load_data,load
import time
import os
...
def local_global_loss_(l_enc, g_enc, batch, measure, mask):
    ...
    Returns:
        torch.Tensor: Loss.
    '''
    num_graphs = g_enc.shape[0]  # 32
    num_nodes = l_enc.shape[0]  #50
    max_nodes = num_nodes // num_graphs

    pos_mask =  torch.zeros((num_nodes, num_graphs)).to(device) #.cuda() #torch.eye(num_nodes) * 0#
...
def train(dataset, gpu, num_layer=4, epoch=40, batch=64):
    ...
    if os.path.exists(f'old_{hid_units}_{batch_size}_IVN_{max_nodes}_ROAD_upgrade_last2_edge.pkl'):
        print('weight exist! loading')
        model.load_state_dict(torch.load(f'old_{hid_units}_{batch_size}_IVN_{max_nodes}_ROAD_upgrade_last2_edge.pkl'))
    ...
    for epoch in range(nb_epochs):
        ...
        if epoch_loss < best:
            ...
            best_t = epoch
            cnt_wait = 0
            torch.save(model.state_dict(), f'{hid_units}_{batch_size}_IVN_{max_nodes}_ROAD_upgrade_last2_edge.pkl')
        else:
            cnt_wait += 1
