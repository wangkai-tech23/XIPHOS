...
class Model(nn.Module):
    def __init__(self, n_in, n_h, num_layers):
        ...
        self.gnn1 = GCN(n_in, n_h, num_layers).to(device)
        self.gnn2 = GCN(n_in, n_h, num_layers).to(device)
...
def local_graph_loss_(l_enc1, l_enc2, batch, measure, mask):
    ...
    pos_mask = torch.full([num_nodes, num_nodes], 1).to(device)
...
def train(dataset, gpu, num_layer=4, epoch=40, batch=64):
    ...
    model = Model(ft_size, hid_units, num_layer).to(device)
    ...
    for epoch in range(nb_epochs):
        ...
        epoch_loss = 0.0
        train_idx = np.arange(adj.shape[0]).to(device)
        ...
        for idx in range(0, len(train_idx), batch_size):
            ...
            batch_idxes = train_idx[idx: idx + batch_size].to(device)
            mask = num_nodes
            ...
            loss1 = local_graph_loss_(lv1.to(device), lv2.to(device), batch, 'JSD', mask)
            loss2 = global_graph_loss_(gv1.to(device), gv2.to(device), batch, 'JSD', mask)
        ...
        if epoch_loss < best:
            ...
            best_t = epoch
            cnt_wait = 0
            torch.save(model.state_dict(), f'Show_{hid_units}IVN_upgrade_final_{batch_size}.pkl')
        else:
            cnt_wait += 1
